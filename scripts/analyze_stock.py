...
def analyze_with_claude(
    ...
    prompt = f"""아래 주어진 데이터만 사용하여 JSON을 반환하세요. 웹 검색 금지. 추가 설명 금지. 오직 JSON만 출력하세요.
...
- 뉴스: {news_text if news_text else "없음"}

## 출력 규칙
- 반드시 아래 JSON 형식만 출력
- ```나 설명 텍스트 절대 포함 금지
- 한국어로 작성
- 네이버 블로그 독자 대상, 친근한 문체

{{"title":"SEO 최적화 제목","summary":"3줄 요약 (줄바꿈은 \n 사용)","body":"본문 마크다운 (주가동향200자+뉴스분석400자+투자포인트300자+전망200자+면책조항)","tags":["태그1","태그2","태그3","태그4","태그5","태그6","태그7","태그8","태그9","태그10"],"sentiment":"긍정 또는 중립 또는 부정","key_points":["핵심1","핵심2","핵심3"]}}"""

    message = client.messages.create(
        model="claude-sonnet-4-6",
        max_tokens=2000,
        system="당신은 JSON만 출력하는 주식 분석 봇입니다. 웹 검색을 하지 않습니다. 설명, 마크다운 펜스, 추가 텍스트 없이 순수 JSON만 반환합니다.",
